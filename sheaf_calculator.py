...
#             yinfer[idx] = nnet.label_inference(
#                 xembed[idx_node[:], :],
#                 wgraph,
#                 idx_target,
#                 idx_node
...
def infer_labels(nnet, xembed, eindex):
    device = next(nnet.parameters()).device

    if torch.is_tensor(xembed):
        xembed_t = xembed.to(device=device, dtype=torch.float64)
    else:
        xembed_t = torch.as_tensor(xembed, dtype=torch.float64, device=device)

    if torch.is_tensor(eindex):
        edge_index_t = eindex.to(device=device, dtype=torch.long)
    else:
        edge_index_t = torch.as_tensor(eindex, dtype=torch.long, device=device)

    with torch.no_grad():
        ynumer = nnet.infer_all_labels(xembed_t, edge_index_t)

    return ynumer.cpu().numpy()



def compute_neural_network_parameters(nnet, nepoch, nbatch, lr,
                                      xembed, ylabel, ylprob, eindex, eglist,
    ...
    optimizer = optim.Adam(nnet.parameters(), lr=lr, weight_decay=5e-4)
    ...
    if torch.is_tensor(xembed):
        xembed = xembed.double()
    else:
        xembed = torch.from_numpy(xembed).double()

    if torch.is_tensor(ylabel):
        ylabel = ylabel.long()
    else:
        ylabel = torch.from_numpy(ylabel).long()

    if torch.is_tensor(ylprob):
        ylprob = ylprob.double()
    else:
        ylprob = torch.from_numpy(ylprob).double()
    ...
    for kepoch in range(nepoch):
        ...
        loss_orth, loss_cons, loss_smap, loss_lbpr, loss_accs = nnet(
            Variable(xembed[idx_node[:], :]),
            ylabel[idx_node[:]],
            ylprob[idx_node[:]],
            wgraph,
            idx_subsample_valid,
        ...
        kapp_orth, kapp_cons, kapp_smap, kapp_lbpr = compute_loss_target(
            loss_orth, loss_cons, loss_smap, loss_lbpr,
            kapp_orth, kapp_cons, kapp_smap, kapp_lbpr,
            kappa, lr
        )

        w_orth, w_cons, w_smap, w_lbpr = compute_loss_weights_simple(
            loss_orth, loss_cons, loss_smap, loss_lbpr,
            kapp_orth, kapp_cons, kapp_smap, kapp_lbpr
        )

        loss = w_orth * loss_orth + w_cons * loss_cons + alpha_smap * w_smap * loss_smap + w_lbpr * loss_lbpr
        ...
        if idx_subsample_ttest.size == 0 and kepoch % 50 == 0:
            print(f"[debug] empty ttest idvert at epoch {kepoch}")
        ...
        if idx_subsample_train.size == 0:
            if kepoch % 50 == 0:
                print(f"[debug] empty train idvert at epoch {kepoch}")
            continue
    ...
    idx_subsample_valid = compute_index_subsample(idx_node, idx_valid)
    if idx_subsample_valid.size == 0 and kepoch % 50 == 0:
        print(f"[DBG] empty valid idvert at epoch {kepoch}")
    ...
    ynumer = infer_labels(nnet, xembed, eindex)
