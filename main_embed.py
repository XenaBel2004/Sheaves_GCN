...
def learn_sheaf_parameters(xembed, ylabel, ylprob, eglist,
    ...
    nnet = Sheaf_NNet(nvert, dimx, nlab, nconv=6, t=1.0, k=2, K=5).double()
...
def main():
    ...
    for ireal in range(nreal):
        ...
        if first_time_flag:
            xembed, eglist, ylabel, ylprob, xsvd = read_data(
            ...
            idx_train, idx_ttest, idx_valid = perform_ttv_split(
                nsample, ftrain=0.8, fttest=0.1, fvalid=0.1
            )
            pca = PCA(n_components = 100)
            xembed = pca.fit_transform(xembed)
            print(pca.explained_variance_ratio_.sum())
            print(f"[PCA] Исходная форма: {pca.n_features_in_}, после PCA: {xembed.shape[1]}")
            np.save("./data/xembed_pca.npy", xembed)
        ...
        learn_sheaf_parameters(
            xembed, ylabel, ylprob, eglist,
            idx_train, idx_ttest, idx_valid,
            dimy=xembed.shape[1],   # теперь dimy=2
            nbatch=512, nepoch=(64 * 128),
