...
def learn_sheaf_parameters(xembed, ylabel, ylprob, eindex, eglist,
                           idx_train, idx_ttest, idx_valid,
                           dimy=16, nbatch=128, nepoch=2048,
                           lr=5.0e-4, unique_name=''):
    print('learning_the_sheaf')
    nvert, dimx = xembed.shape
    nlab = np.int64(np.max(ylabel) + 1)
    nnet = Sheaf_NNet(nvert, dimx, nlab, t=1.0, k=2, K=5)

    print("DIMX:", dimx, "----------------")

    ltrain, lttest, lvalid, accs_train, accs_ttest, accs_valid = compute_neural_network_parameters(
        nnet, nepoch, nbatch, lr,
...
def main():
    ...
    for ireal in range(nreal):
        ...
        set_seed(ireal)
        xembed, eindex, eglist, ylabel, ylprob, xsvd, idx_train, idx_ttest, idx_valid = read_data(
        ...
        learn_sheaf_parameters(
            xembed, ylabel, ylprob, eindex, eglist,
            idx_train, idx_ttest, idx_valid,
            dimy=xembed.shape[1],
            nbatch=512, nepoch=4096,
            lr=5.0e-4, unique_name=unique_name
        )
